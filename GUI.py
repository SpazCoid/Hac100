...
#---DATA ENTRY
    ...
    frameButtonMA = Frame(root)
    frameButtonMA.pack()
    frameButtonMA.configure(background="#cdfbff")

#---BUTTON TO REDIRECT USER TO ACCOUNT CREATION
    buttn3 = Button(frameButtonMA,height=5, width=16 ,background="#7f82ff" ,text = "Log Out", command=lambda: [StartUp(), frameButtonMA.destroy(), frameMedMenu.destroy()]).grid(row=5,column=0)
    ...
    for record in records:
        print("Medication ID = ", record[0] , "Medication Name = ", record[1] , "Medication Description = ", record[2] , "Medication Dose = ", record[3])
